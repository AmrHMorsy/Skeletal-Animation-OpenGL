...
FBXModel::FBXModel( std::string modelfilepath, std::vector<std::string> texturesfilepaths, glm::mat4 v, glm::mat4 proj, glm::vec3 c, std::string vsfilepath, std::string fsfilepath, glm::mat4 lightview, glm::mat4 lightproj, glm::vec3 lightpos, glm::vec3 lightcol): view(v), projection(proj), camera(c), model_filepath(modelfilepath), file_paths(texturesfilepaths), vs_filepath( vsfilepath), fs_filepath( fsfilepath ), light_view(lightview), light_projection(lightproj), light_position(lightpos), light_color(lightcol)
{
...
void FBXModel::initialize_variables()
{
    position = 0.0f ;
    time = 0.0f ;
    elapsed_time = 0.0f ;
...
void FBXModel::load_FBXModel()
{
    parser = new Parser() ;
    parser->parse_fbx( model_filepath ) ;
    meshes = parser->get_meshes() ;
    num_meshes = meshes.size() ;
    animator = new Animator( parser->get_root_bone(), meshes, parser->get_num_keyframes(), parser->get_precomputed_animations(), parser->get_animation_mapping() ) ;
}

void FBXModel::load_shaders()
{
...
void FBXModel::set( Skybox * skybox )
{
    this->skybox = skybox ;
}

void FBXModel::generate_buffers()
{
...
void FBXModel::render( unsigned int shadow_shader_program )
{
...
void FBXModel::render( Shadow * shadow )
{
    if( animator->animate() ){
        bind_vertices() ;
...
void FBXModel::set_uniform_variables()
{
...
void FBXModel::update_view( glm::vec3 camera, glm::mat4 v )
{
    this->camera = camera ;
    glUseProgram(shader_program) ;
    glUniform3fv(glGetUniformLocation(shader_program, "camera"), 1, &camera[0]) ;
...
void FBXModel::activate_textures()
{
...
void FBXModel::set_animation( std::string animation )
{
    animator->set_animation( animation ) ;
}

void FBXModel::set_animation( int animation, float requested_steer_angle )
{
...
void FBXModel::generate_error( std::string msg )
{
