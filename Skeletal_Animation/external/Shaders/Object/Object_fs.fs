...
vec4 pbr()
{
    ...
    float roughness = texture(roughness_map, texture_coordinates).g + 0.8f ;
    float metallic  = texture( metallic_map, texture_coordinates).b ;
...
float compute_fog()
{
    ...
    float distance = 0.02*length(world_position - camera); // 0.03
