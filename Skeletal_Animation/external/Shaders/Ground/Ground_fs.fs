...
float compute_shadow()
{
    ...
    for (int x = -samples; x <= samples; ++x){
        for (int y = -samples; y <= samples; ++y){
            vec2 offset = vec2(x, y) * sample_radius;
            float closest_depth = texture(shadow_map, normalized_device_coordinates.xy + offset).r;
...
vec4 pbr()
{
    ...
    float roughness = texture(roughness_map, texture_coordinates).r + 0.8f ;
    float ao = texture(ao_map, texture_coordinates).r ;
    vec3 normal = calculate_normal() ;
    vec3 view = normalize(camera - world_position) ;
    vec3 radiance = calculate_radiance( normal, view, albedo, roughness ) * compute_shadow() ;
...
float compute_fog()
{
    ...
    float distance = 0.03*length(world_position - camera) ;
...
void main()
{
    fragment = mix( vec4(vec3(0.5f), 1.0f), pbr(), compute_fog()) ;
}
